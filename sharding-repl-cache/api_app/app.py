...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", {}):
            shard_details = {}
            shard_details["hosts"] = shard["host"]
            repl_count = len(shard["host"].split(','))
            shard_details["repl_count"] = repl_count
            shards[shard["_id"]] =shard_details
    ...
    for collection_name in collection_names:
        ...
        collection_info = {
            ...
            "shard_details": {}  # Для хранения информации о шардированных документах

        }
        # Получаем данные о документах по шардовым узлам
        if topology_type == "Sharded":
            shard_stats = await collection.aggregate([
                {"$collStats": {"storageStats": {}}},
                {"$project": {"shard": "$shard", "count": "$storageStats.count"}}
            ]).to_list(None)

            # Обрабатываем результаты и добавляем в shard_details
            for stat in shard_stats:
                shard_id = stat.get("shard")
                count = stat.get("count", 0)
                collection_info["shard_details"][shard_id] = count
